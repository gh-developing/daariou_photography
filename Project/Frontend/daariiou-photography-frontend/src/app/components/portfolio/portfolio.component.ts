...
export class PortfolioComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.splitIntoThreelists();
  }

  splitIntoThreelists() {
    var oneThirhirdOfAllImages = Math.floor(+this.allImages.length / 3);
    var middleList = this.allImages.length - (oneThirhirdOfAllImages*2)
    for (var i = 0; i<oneThirhirdOfAllImages; i++){
      this.gallery1[i] = this.allImages[i];
    }
    for (var i = 0; i<oneThirhirdOfAllImages; i++){
      this.gallery3[i] = this.allImages[i+oneThirhirdOfAllImages];
    }
    for (var i = 0; i<middleList; i++){
      this.gallery2[i] = this.allImages[i+(oneThirhirdOfAllImages*2)];
    }
  }
